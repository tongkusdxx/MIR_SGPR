...
class MEDFE(BaseModel):
    ...
    def set_input(self, input_De, input_St, mask):
        ...
        if mask.dim() == 5:
            batch_size, channels, depth, height, width = mask.size()


        # 如果 mask 是五维张量，去除 depth 维度
        if mask.dim() == 5:
            batch_size, channels, depth, height, width = mask.size()

            mask = mask.view(batch_size, channels * depth, height, width)  # 这里将 depth 合并到 channels 上


        if mask.size(2) != self.Gt_DE.size(2) or mask.size(3) != self.Gt_DE.size(3):
            #print("Adjusting mask size to match the image size")
            mask = F.interpolate(mask, size=(self.Gt_DE.size(2), self.Gt_DE.size(3)), mode='nearest')
        ...
        # ȷ�� self.mask_global �ڵ���ά�ĳߴ��� self.input_DE ��ƥ��
        mask_global_narrowed = self.mask_global.narrow(1, 0, 1).narrow(2, 0, 256).bool()

# ʹ�õ������ mask_global_narrowed �������� masked_fill_ ����
        self.input_DE.narrow(1, 0, 1).masked_fill_(mask_global_narrowed, 2 * 123.0 / 255.0 - 1.0)
        self.input_DE.narrow(1, 1, 1).masked_fill_(mask_global_narrowed, 2 * 104.0 / 255.0 - 1.0)
        self.input_DE.narrow(1, 2, 1).masked_fill_(mask_global_narrowed, 2 * 117.0 / 255.0 - 1.0)
    ...
    def forward(self):
        ...
        reversed_mask_image = self.fake_out * self.mask_global
        self.fake_out = reversed_mask_image + self.input_DE * (1 - self.mask_global)
    ...
    def backward_G(self):
        ...
        # Calculate reversed_mask_loss
        mask = real_AB[:, -1:, :, :]  
        reversed_mask = self.reverse_mask(mask)
        input_img = real_AB[:, :3, :, :]  
        output_img = fake_AB[:, :3, :, :]

        
        reversed_mask = reversed_mask.expand(-1, 3, -1, -1)
        
        masking = reversed_mask * input_img
        predicting = reversed_mask * output_img
        
        vgg_input = torch.cat([masking, predicting], dim=0)
        
        feature_outputs = self.vgg(vgg_input)['relu4_1']
        
        
        feature_masking = feature_outputs[:input_img.size(0)]
        feature_predicting = feature_outputs[input_img.size(0):]
        reversed_mask_loss = F.mse_loss(feature_masking, feature_predicting)
        ...
        self.loss_G = self.loss_L1 * self.opt.lambda_L1 + self.loss_G_GAN * self.opt.lambda_Gan + \
                      self.Perceptual_loss * self.opt.lambda_P + self.Style_Loss * self.opt.lambda_S+reversed_mask_loss * 0.8
