...
class BiGFF(nn.Module):
    '''Bi-directional Gated Feature Fusion for multi-scale.'''
...
class PCconv(nn.Module):
    def __init__(self):
        ...
        self.activation = nn.LeakyReLU(negative_slope=0.2)
        #self.bigff = BiGFF(in_channels= 256, out_channels= 256)
    ...
    def forward(self, input, mask):
        ...
        x_ST = self.down(x_ST)
        x_DE = self.down(x_DE)
        #x_DE, x_ST = self.bigff(x_DE, x_ST)
        x_ST = [x_ST, mask_1]
        x_DE = [x_DE, mask_1]
        ...
        #x_DE_fi, x_ST_fi = self.bigff(x_DE_fi, x_ST_fi)
